import torch
from tqdm import tqdm, trange
from typing import Optional, Dict
import os
from itertools import islice

from torch.utils.data import DataLoader

from .world_flow_sampler import world_flow_sample
import aim
from aim import Run
import numpy as np

from ..metrics import compute_kid_from_tensors, load_inception
from ..utils import images_dict_to_grid

inception = load_inception()

device = "cuda" if torch.cuda.is_available() else "cpu"


def generate_world_flow_snapshots(
    model, dataset, steps_to_show=[8, 4, 2, 0]
) -> Dict[int, np.ndarray]:
    num_steps = 10

    # [B, C, H, W]
    x0 = torch.randn(1, 3, 64, 64).to(device)
    # [1, C, H, W]
    start = dataset[0][0].to(device).unsqueeze(0)
    # [B]
    action = torch.full((x0.size(0),), dataset[0][1], device=device)

    flow_snapshots = world_flow_sample(
        x0, model, start, action, num_steps, steps_to_show
    )
    return flow_snapshots, start.squeeze(0), action.float().mean().int().item()


def generate_world_flow_rollout(
    model,
    dataset,
    rollout_steps: int = 5,
    device: str = "cuda",
):
    num_steps = 10
    start, action, end = dataset[0]
    start, end = start.to(device), end.to(device)
    rollout_snapshots = {"start": start, "end": end}
    # [1]
    action_tensor = torch.full((1,), action, device=device)

    final_step = num_steps - 1

    steps_to_show = [final_step]

    prev = start.unsqueeze(0)
    for i in range(rollout_steps):
        # [B, C, H, W]
        x0 = torch.randn(1, 3, 64, 64).to(device)

        flow_snapshots = world_flow_sample(
            x0, model, prev, action_tensor, num_steps, steps_to_show
        )
        rollout_snapshots[i] = flow_snapshots[final_step]
        prev = flow_snapshots[final_step].unsqueeze(0) * 255
    return rollout_snapshots, action


def eval_world_flow(model, loader, t_val=None):
    model.eval()
    with torch.inference_mode():
        starts, actions, imgs = loader.dataset[0]

        # [1, C, H, W]
        x1 = (imgs.to(device) / 127.5) - 1
        x1 = x1.unsqueeze(0)

        B = x1.shape[0]
        if t_val is None:
            t = torch.rand(B, device=device)
        else:
            t = torch.full((B,), t_val, device=device)

        x0 = torch.randn_like(x1)
        v = x1 - x0
        xt = x0 + v * t

        # [1, C, H, W]
        starts = (starts.to(device) / 127.5) - 1
        starts = starts.unsqueeze(0)
        # [1]
        actions_float = torch.tensor([actions]).to(device).float() / 4
        with torch.autocast(device, torch.float16):
            v_pred = model(xt, t, starts, actions_float)

        x1_hat = x0 + v_pred

        # unnormalize for visualization
        def denorm(x):
            x = x.clamp(-1, 1)
            x = (x + 1) / 2
            return x.squeeze(0)

        eval_dict = {
            "starts": denorm(starts),
            "x1": denorm(x1),
            "x1_hat": denorm(x1_hat),
        }

    return eval_dict, v_pred, v, actions


def sample_world_flow_batch(
    model,
    dataloader,
    num_samples: int,
    num_steps: int = 10,
    batch_size: int = 64,
    device: str = "cuda",
):
    """
    Generate num_samples images from the diffusion model using DDIM.
    Returns [N, 3, H, W] in [-1, 1].
    """
    model.eval()
    all_samples = []

    assert num_samples % dataloader.batch_size == 0
    max_batches = num_samples // dataloader.batch_size

    final_step = num_steps - 1

    with torch.inference_mode():
        for _, (starts, actions, ends) in enumerate(
            tqdm(islice(dataloader, max_batches))
        ):
            x0 = torch.randn(starts.size(0), 3, 64, 64, device=device)
            # Only care about final x_0
            snapshots = world_flow_sample(
                x0,
                model,
                starts.to(device),
                actions.to(device),
                num_steps,
                steps_to_show=[final_step],
            )
            x0_hat = snapshots[final_step]  # assume dict {step: tensor}

            all_samples.append(x0_hat)

    return torch.cat(all_samples, dim=0)  # [N, 3, H, W]


def eval_world_flow_kid(
    model,
    dataloader,
    inception,
    num_real_samples: int = 512,
    num_steps: int = 10,
    device: str = "cuda",
):
    model.eval()
    real_imgs = []

    with torch.inference_mode():
        for _, _, ends in dataloader:
            ends = ends.to(device).float()
            ends = (ends / 127.5) - 1  # [-1, 1]
            real_imgs.append(ends)

            if sum(x.size(0) for x in real_imgs) >= num_real_samples:
                break

    real_imgs = torch.cat(real_imgs, dim=0)[:num_real_samples]  # [N, 3, H, W]

    fake_imgs = sample_world_flow_batch(
        model=model,
        dataloader=dataloader,
        num_samples=num_real_samples,
        num_steps=num_steps,
        batch_size=dataloader.batch_size,
        device=device,
    )  # [N, 3, H, W] in [-1, 1]

    def denorm(x):
        x = x.clamp(-1, 1)
        x = (x + 1) / 2.0  # [-1, 1] -> [0, 1]
        return x

    real_imgs = denorm(real_imgs)
    fake_imgs = denorm(fake_imgs)

    kid_value = compute_kid_from_tensors(real_imgs, fake_imgs, inception)
    return kid_value


def train_world_flow(
    model,
    criterion,
    optimizer,
    dataloader,
    scheduler: Optional[torch.optim.lr_scheduler._LRScheduler] = None,
    ema: Optional[torch.nn.Module] = None,
    device: str = "cpu",
    num_epochs: int = 10,
    save_dir: str = "./checkpoints",
    run_name: str = "run",
    save_freq: int = 10,
):
    run = Run(experiment="world_flow")
    run.name = run_name
    run["hparams"] = {
        "num_epochs": num_epochs,
        "save_freq": save_freq,
        "device": device,
        "batch_size": dataloader.batch_size,
        "lr": optimizer.defaults["lr"],
        "dim": model.dim,
    }

    save_dir = f"{save_dir}/{run_name}"
    model.to(device)
    os.makedirs(save_dir, exist_ok=True)

    eval_loader = DataLoader(dataloader.dataset, batch_size=512, shuffle=False)

    scaler = torch.amp.GradScaler(device)

    for i in trange(num_epochs):
        model.train()

        epoch_loss = 0.0

        for j, (starts, actions, ends) in enumerate(tqdm(dataloader)):
            starts, actions, ends = (
                starts.to(device).float(),
                actions.to(device).float(),
                ends.to(device).float(),
            )
            starts = (starts / 127.5) - 1
            ends = (ends / 127.5) - 1

            optimizer.zero_grad()

            x0 = torch.randn_like(ends)
            x1 = ends
            t = torch.rand(
                x0.size(0),
            ).to(device)

            # [B] -> [B, 1, 1, 1]
            t_expand = t.view(-1, 1, 1, 1)

            xt = x0 + (x1 - x0) * t_expand
            v = x1 - x0

            actions_float = actions.float() / 4

            with torch.autocast(device, torch.float16):
                v_pred = model(xt, t, starts, actions_float)
                loss = criterion(v_pred, v)

            global_step = i * len(dataloader) + j
            run.track(loss.item(), name="training_loss", step=global_step)
            epoch_loss += loss.item()

            scaler.scale(loss).backward()
            scaler.step(optimizer)
            scaler.update()
            # optimizer.step()

            if ema is not None:
                ema.update(model)

        if scheduler is not None:
            scheduler.step()

        epoch_loss /= len(dataloader)

        if (i + 1) % save_freq == 0:
            print(f"Epoch {i+1}/{num_epochs}, Loss: {epoch_loss}")
            torch.save(model.state_dict(), f"{save_dir}/model_{i+1}.pth")

            flow_snapshots, start, action = generate_world_flow_snapshots(
                model, dataloader.dataset
            )
            steps_str = "_".join(map(str, flow_snapshots.keys()))
            flow_snapshots["start"] = start
            steps_str += f"_start_{action}"
            imgs = images_dict_to_grid(flow_snapshots)
            run.track(
                aim.Image(imgs, caption=f"flow_snapshot_{steps_str}"),
                name="flow_snapshot",
                step=global_step,
            )

            rollout_snapshots, action = generate_world_flow_rollout(
                model, dataloader.dataset
            )
            steps_str = "_".join(map(str, rollout_snapshots.keys()))
            steps_str += f"_action_{action}"
            imgs = images_dict_to_grid(rollout_snapshots)
            run.track(
                aim.Image(imgs, caption=f"rollout_{steps_str}"),
                name="rollout",
                step=global_step,
            )

            eval_dict, v_pred, v, actions = eval_world_flow(
                model, eval_loader, t_val=0.625
            )
            steps_str = "_".join(map(str, eval_dict.keys()))
            eval_imgs = images_dict_to_grid(eval_dict)
            run.track(
                aim.Image(eval_imgs, caption=f"eval_{steps_str}_{actions}"),
                name="eval",
                step=global_step,
            )

            loss = criterion(v_pred, v)
            run.track(loss.item(), name="eval_loss", step=global_step)
            kid_value = eval_world_flow_kid(model, eval_loader, inception)
            run.track(kid_value, name="kid", step=global_step)
